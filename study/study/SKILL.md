...
python3 scripts/extract_source.py --input chapter.html --output-dir ./extracted
```
...
1. **Run the extraction script** (or parse manually if preferred).
   - For PDF: extracts text with layout, preserves page numbers, detects headings, lists, tables,
     and saves embedded images.
   - For HTML: parses the DOM, preserves semantic structure, strips ads/nav/footers.
2. **Review the auto-generated reports.**
   - Check `extraction-report.md` for page/image/table counts.
   - Review `corrections.md` for flagged issues.
3. **Detect and flag additional errors.**
...
- `scripts/extract_source.py`
  - Extracts structured text, images, and tables from PDF or HTML source files.
  - Outputs `chapters.json`, `assets/`, `extraction-report.md`, and `corrections.md`.
  - Dependencies: `pip install pymupdf beautifulsoup4 pdfplumber Pillow`
